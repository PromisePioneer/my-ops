...
class InvoiceService
{
    ...
    {
        $invoice = Invoice::where('branch_id', $request->user()->branch_id)->where('contact_id', $request->contact_id)->latest()->first();
    ...
    public function store(InvoiceRequest $request): void
    {
        DB::transaction(function () use ($request) {
            ...
            $data['cooperative_contract_file'] = $this->handleFileUploadService->upload($request, 'documents/invoice/cooperative-contract', 'cooperative_contract_file');
    ...
    public function update(InvoiceRequest $request, Invoice $invoice): void
    {
        DB::transaction(function () use ($request, $invoice) {
            ...
            $data['branch_id'] = $request->user()->branch_id;
            $data['invoice_number'] = self::generateInvoiceNumber($request);
            $data['baa_file'] = $this->handleFileUploadService->upload($request, 'documents/invoice/baa', 'baa_file', $invoice->baa_file);
            $data['cooperative_contract_file'] = $this->handleFileUploadService->upload($request, 'documents/invoice/cooperative-contract', 'cooperative_contract_file', $invoice->cooperative_contract_file);
            $data['created_by'] = $request->user()->id;
            $data['grand_total'] = $request->grand_total;
            $invoice->update($data);
    ...
    public function accountTransactionAfterInvoiceSent(Invoice $invoice): void
    {
        ...
        $description = sprintf(self::INVOICE_SENT_DESCRIPTION, $selectedContact['company_name'], $invoice->invoice_number);

        DB::transaction(function () use ($piutangPelanggan, $description, $invoice) {
            $this->accountTransactionService->createDebitTransaction($description, $invoice->grand_total, null, $piutangPelanggan->id);
    ...
    public function calculatePPNAccountTransactionAfterInvoiceSent(Invoice $invoice): void
    {
        ...
        $description = sprintf(self::CALCULATE_PPN_AFTER_INVOICE_SENT_DESCRIPTION, $selectedContact['company_name'], $invoice->invoice_number);
        ...
        DB::transaction(function () use ($ppn, $piutangPelanggan, $description, $totalPlusTax) {
            $this->accountTransactionService->createDebitTransaction($description, $totalPlusTax, null, $piutangPelanggan->id);
    ...
    public function updatePaymentStatus(Request $request, Invoice $invoice): void
    {
        ...
        DB::transaction(function () use ($request, $invoice, $selectedContact, $description, $rekeningMayatamaPusat, $piutangPelanggan, $currentPPN) {
            ...
            $this->accountTransactionService->createDebitTransaction($description, $totalAfterInvoicePaid, null, $rekeningMayatamaPusat->id);
            $this->accountTransactionService->createCreditTransaction($description, $totalAfterInvoicePaid, null, $piutangPelanggan->id);
    ...
    public function includePPh23AfterInvoicePaid(Request $request, Invoice $invoice, string $companyName): void
    {
        ...
        $piutangPelanggan = $this->subAccount->findPiutangPelangganSubAccount($invoice->branch_id);
        $description = sprintf(self::INCLUDE_PPH23_AFTER_INVOICE_PAID_DESCRIPTION, $companyName, $invoice->invoice_number);

        DB::transaction(function () use ($pph23Account, $piutangPelanggan, $description, $request) {
            $this->accountTransactionService->createDebitTransaction($description, $request->pph23_form, null, $pph23Account->id);
