                    ...
                    <div class="card-body p-12">
                            ...
                            <div class="col-lg-6">
                                <div class="d-flex justify-content-end fw-row text-nowrap order-1 order-xxl-2 me-4"
                                    ...
                                    <input type="text" name="invoice_number"
                                           ...
                                           placeholder="INVOICE" value="{{ $invoice->invoice_number }}" readonly/>
                                </div>
                            </div>
                        </div>
                                ...
                                <div class="form-group row mb-6">
                                    ...
                                    <div class="col-lg-11 fv-row">
                                        <select name="account_id" id="selectedAccount"
                                                class="form-select form-select-solid akunSearch"
                                                data-placeholder="Select an option">
                        ...
                        <div class="mb-10">
                            ...
                            <textarea name="description" class="form-control form-control-solid" rows="3"
                                      placeholder="Thanks for your business">{{ $invoice->description }}</textarea>
                        </div>
                    </div>
        ...
        function generateInvoice() {
            return {
                ...
                async init() {
                    ...
                    await this.selectedInvoiceServices();
                },
                async selectedContact() {
                    ...
                    $.ajax({
                        type: 'GET',
                        dataType: "JSON",
                        url: `/income-transactions/invoice/contact/selected/${this.id}`,
                ...
                async selectedAccount() {
                    ...
                    $.ajax({
                        type: 'GET',
                        dataType: "JSON",
                        url: `/income-transactions/invoice/account/selected/${this.id}`,
                    }).then(function (response) {
                        const option = new Option(response.name, response.id, true, true);
                        selectedAccount.append(option).trigger('change');
                ...
                async selectedInvoiceServices() {
                    const resp = await axios.get(`/income-transactions/invoice/services/selected/${this.id}`);
                    this.$nextTick(() => {
                        resp.data.map((resp, i) => {
                            this.fields.push({
                ...
                async generateInvoice() {
                    this.buttonLoading = true;
                    try {
                        await axios.post(`/income-transactions/invoice/update/${this.id}`, new FormData(this.form))
