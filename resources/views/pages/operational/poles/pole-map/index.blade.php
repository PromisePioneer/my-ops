...
@section('content')
    ...
    <div class="d-flex flex-column flex-xl-row" x-data="polesMap()">
        ...
        function polesMap() {
            return {
                ...
                async init() {
                    ...
                    if (this.poleData.length > 0) {
                        this.map = L.map('map').setView([this.poleData[0].lat, this.poleData[0].long], 16);
                        this.map.invalidateSize();
                        this.mapTileLayer(this.map);
                        await this.poleMarker(this.map);
                    }
                },
                async getMonth() {
                    this.months.push(
                ...
                async poleMarker(map) {
                    this.poleData.forEach((pole) => {
                        L.marker(L.latLng(pole.lat, pole.long), {
                ...
                async getODPData() {
                    this.isLoading = true;
                    try {
                        const resp = await axios.get('/operational/poles-map/data');
                        this.poleData = resp.data;
                    } catch (e) {
                        console.log(e)
                    } finally {
                ...
                download() {
                    ...
                    const geojsonFeature = {
                        ...
                        "features": this.poleData.map(pole => {
                            return {
                                "type": "Feature",
                                "properties": {
                                    "name": pole.code,
                                ...
                                "geometry": {
                                    "type": "Point",
                                    "coordinates": [pole.long, pole.lat]
                                }
                            };
                        })
                    };
                ...
                async filter() {
                    ...
                    try {
                        this.map = null;
                        const resp = await axios.get('/operational/poles-map/filter', {
                            params: {
                                month: month,
                                year: year,
                            }
