                ...
                <div class="py-5">
                    <div class="table-responsive">
                        <table class="table align-middle table-bordered fs-6 gy-5 table-striped" id="kt_table_users">
                            ...
                            <template x-if="!isLoading && poles.data?.length === 0">
                                <tbody class="fw-bold">
                                <tr>
                                    <td colspan="9">
                            ...
                            <template x-for="pole in poles?.data" :key="pole.id">
                                <tbody class="fw-bold">
                                <tr>
                                    <td>
                                        <div class="form-check form-check-sm form-check-custom form-check-solid"
                                             @click="selectCheckBox($event)">
                                            <input class="form-check-input" type="checkbox" :value="pole.id"
                                                   :id="'checkbox-' + pole.id"/>
                                        </div>
                                    </td>
                    ...
                    <ul class="pagination float-end mb-4 mt-4">
                        <template x-for="pagination in poles.links">
        ...
        function polesData() {
            return {
                ...
                async getPolesData() {
                    this.isLoading = true;
                    try {
                        const resp = await axios.get('/operational/poles/data');
                        this.poles = resp.data;
                    } catch (e) {
                        console.log(e);
                    } finally {
                ...
                async save() {
                    this.buttonLoading = true;
                    try {
                        await axios.post('/operational/poles/', new FormData(this.formCreate))
                ...
                async edit(id) {
                    const resp = await axios.get(`/operational/poles/${id}`);
                ...
                async update(id) {
                    this.buttonLoading = true;
                    try {
                        await axios.post(`/operational/poles/${id}`, new FormData(this.formEdit))
                        await showAlert('success', 'Data berhasil disimpan')
                        this.formEdit.reset();
                        this.modalEdit.hide();
                        await this.init();
                ...
                async destroy() {
                    showConfirmModal("Anda yakin?", "Data akan hilang.", "Ya, Hapus!", async () => {
                        try {
                            await axios.post(`/operational/poles/destroy`, new FormData(this.formDelete));
