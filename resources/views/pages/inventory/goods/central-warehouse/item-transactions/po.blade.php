        ...
        <div class="py-5">
            <div class="table-responsive">
                <table class="table align-middle table-bordered table-row-dashed fs-6 gy-5" id="kt_table_users">
                    ...
                    <template x-if="!isLoading && poItem.data?.length === 0">
                        <tbody class="fw-bold">
                        <tr>
                            <td colspan="9">
                    ...
                    <template x-for="(item, index) in poItem?.data" :key="item.id">
                        ...
                        <tr>
                            ...
                            <td>
                                <template x-if="item.item.need_sn === 1 && item.status === 1 && item.from_po === 1">
                                    <a :href="`/inventory/list-of-items/item-transactions/po-data/detail/${item.id}`"
            ...
            return {
                ...
                async paginationEndPoint(url) {
                    if (url) {
                        ...
                        this.poItem = resp.data
                    }
                },
                async searchData() {
                    this.isLoading = true;
                    try {
                        const resp = await axios.get('/inventory/list-of-items/item-transactions/po-data/search', {
                ...
                async getPoData() {
                    this.isLoading = true;
                    try {
                        const resp = await axios.get('/inventory/list-of-items/item-transactions/po-data');
                        this.startIndex = resp.data.from
                        this.poItem = resp.data;
                    } catch (e) {
                        console.log(e);
