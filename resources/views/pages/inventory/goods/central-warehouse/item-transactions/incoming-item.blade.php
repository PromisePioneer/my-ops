        ...
        <div class="py-5">
            <div class="table-responsive">
                <table class="table align-middle table-bordered table-row-dashed fs-6 gy-5" id="kt_table_users">
                    ...
                    <template x-if="!isLoading && itemTransactions.data?.length === 0">
                        <tbody class="fw-bold">
                        <tr>
                            <td colspan="9">
                    ...
                    <template x-for="(item, index) in itemTransactions?.data" :key="item.id">
                        ...
                        <tr>
                            ...
                            <td x-text="`${item.qty} ${item.item_unit_type}`"></td>
        ...
        function incomingItem() {
            return {
                ...
                async getItemTransactions() {
                    this.isLoading = true;
                    try {
                        const resp = await axios.get('/inventory/list-of-items/item-transactions/incoming-items-data');
                        this.startIndex = resp.data.from
                        this.itemTransactions = resp.data;
                    } catch (e) {
                        console.log(e);
