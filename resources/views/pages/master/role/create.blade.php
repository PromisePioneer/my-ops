                ...
                <form id="form" @submit.prevent="save()">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row justify-content-center align-items-center mb-6">
                            <template x-for="row in permission.data" :key="row.id">
            ...
            return {
                ...
                async nextPage() {
                    if (this.permission.next_page_url) {
                        const resp = await axios.get(`${this.permission.next_page_url}`);
                        this.permission = resp.data
                    }
                },
                async previousPage() {
                    if (this.permission.prev_page_url) {
                        const resp = await axios.get(`${this.permission.prev_page_url}`);
                        this.permission = resp.data
                    }
                },
                async save() {
                    ...
                    try {
                        await axios.post('/master/roles', new FormData(this.form))
