...
@section('content')
    <div x-data="branchesData()">
        <div class="card card-xl-stretch mb-5 mb-xl-8">
            @include('pages.master.broadband-packet.modal.create')
            @include('pages.master.broadband-packet.modal.edit')
        ...
        function branchesData() {
            return {
                ...
                async getBroadbandPacketData() {
                    this.isLoading = true;
                    try {
                        const resp = await axios.get('/master/broadband-packet/data');
                ...
                async searchData() {
                    try {
                        this.broadbandPackets = await axios.get('/master/broadband-packet/search', {
                ...
                async save() {
                    ...
                    try {
                        await axios.post('/master/broadband-packet', new FormData(this.formCreate))
                ...
                async edit(id) {
                    const resp = await axios.get(`/master/broadband-packet/${id}`);
                    this.editVal = resp.data;
                },
                async update(id) {
                    this.buttonLoading = true;
                    try {
                        await axios.post(`/master/broadband-packet/${id}`, new FormData(this.formEdit))
                        await showAlert('success', 'Data berhasil disimpan')
                        this.modalEdit.hide();
                        this.formEdit.reset();
                        await this.init();
                ...
                async destroy() {
                    showConfirmModal("Anda yakin?", "Data akan hilang.", "Ya, Hapus!", async () => {
                        try {
                            await axios.post(`/master/broadband-packet/destroy`, new FormData(this.deleteForm));
