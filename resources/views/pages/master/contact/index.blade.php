...
@section('content')
    ...
    <div x-data="contactData()">
        ...
        @include('pages.master.contact.modal.edit')
    ...
    <script defer>
        function contactData() {
            return {
                ...
                async searchData() {
                    try {
                        this.contacts = await axios.get('/master/contact/search', {
                ...
                async edit(id) {
                    const resp = await axios.get(`/master/contact/edit/${id}`);
                    this.editVal = resp.data;
                },
                async update(id) {
                    this.buttonLoading = true;
                    try {
                        await axios.post(`/master/contact/update/${id}`, new FormData(this.formEdit))
                        await showAlert('success', 'Data berhasil disimpan')
                        this.formEdit.reset();
                        this.modalEdit.hide();
                        await this.init();
                ...
                async destroy() {
                    showConfirmModal("Anda yakin?", "Data akan hilang.", "Ya, Hapus!", async () => {
                        try {
                            await axios.post(`/master/contact/destroy`, new FormData(this.deleteForm));
                ...
                async filterByBranch() {
                    ...
                    $(".filter-branch-select2").on('change', async function (e) {
                        ...
                        const response = await axios.get(`/master/contact/filter/branch/data/${selectedBranch.id}`);
                        self.contacts = response.data;
                    });
                },
                async contactData() {
                    const resp = await axios.get('/master/contact/data')
                    this.contacts = resp.data
