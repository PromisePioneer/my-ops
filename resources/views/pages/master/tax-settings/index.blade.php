...
@section('content')
    ...
    <div x-data="taxSettingsData()">
        @include('pages.master.tax-settings.modal.create')
        @include('pages.master.tax-settings.modal.edit')
    ...
    <script>
        function taxSettingsData() {
            return {
                ...
                async init() {
                    const taxSettings = await axios.get('/master/tax-settings/data');
                    this.taxSettings = taxSettings.data;
                    this.isLoading = false;
                },
                async searchData() {
                    this.taxSettings = await axios.get('/master/tax-settings/search', {
                        params: {
                            search: this.search
                        },
                        headers: {
                ...
                async save() {
                    ...
                    try {
                        await axios.post('/master/tax-settings/', new FormData(this.formCreate))
                ...
                async edit(id) {
                    const resp = await axios.get(`/master/tax-settings/${id}`);
                    this.editVal = resp.data;
                },
                async update(id) {
                    this.buttonLoading = true;
                    try {
                        await axios.post(`/master/tax-settings/update/${id}`, new FormData(this.formEdit))
                ...
                async destroy() {
                    showConfirmModal("Anda yakin?", "Data akan hilang.", "Ya, Hapus!", async () => {
                        try {
                            await axios.post(`/master/tax-settings/destroy`, new FormData(this.deleteForm));
