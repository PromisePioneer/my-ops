...
@section('content')
    <div x-data="assetsData()">
        <div class="card card-xl-stretch mb-5 mb-xl-8">
            @include('pages.finance-master-data.assets.modal.create')
            @include('pages.finance-master-data.assets.modal.edit')
            @include('pages.finance-master-data.assets.modal.import')
        ...
        function assetsData() {
            return {
                ...
                async searchData() {
                    this.isLoading = true;
                    try {
                        const response = await axios.get('/finances-master-data/assets/search', {
                ...
                async save() {
                    ...
                    try {
                        await axios.post('/finances-master-data/assets', new FormData(this.formCreate))
                ...
                async edit(id) {
                    const resp = await axios.get(`/finances-master-data/assets/${id}`);
                ...
                async update(id) {
                    this.buttonLoading = true;
                    try {
                        await axios.post(`/finances-master-data/assets/update/${id}`, new FormData(this.formEdit))
                        await showAlert('success', 'Data berhasil disimpan')
                        this.modalEdit.hide();
                        this.formEdit.reset();
                        await this.init();
                ...
                async destroy() {
                    showConfirmModal("Anda yakin?", "Data akan hilang.", "Ya, Hapus!", async () => {
                        try {
                            await axios.post(`/finances-master-data/assets/destroy`, new FormData(this.formDelete));
                ...
                async getAssetsData() {
                    this.isLoading = true;
                    try {
                        const resp = await axios.get('/finances-master-data/assets/data');
                        this.assets = resp.data;
                    } catch (e) {
                        console.log(e)
                    } finally {
                ...
                async importData() {
                    this.buttonLoading = true;
                    try {
                        await axios.post('/finances-master-data/assets/import', new FormData(this.formImport))
                ...
                async getBranchData() {
                    $(".branches-select2").select2({
                        ...
                        ajax: {
                            url: '/finances-master-data/assets/branch/data',
                            dataType: "json",
                            type: "GET",
                ...
                async getDebitAccountData() {
                    $(".debit-accounts-select2").select2({
                        ...
                        ajax: {
                            url: '/finances-master-data/assets/debit-account/data',
                            dataType: "json",
                            type: "GET",
                            data: params => ({search: params.term}),
                            processResults: data => ({results: data}),
                ...
                async getCreditAccountData() {
                    $(".credit-accounts-select2").select2({
                        ...
                        placeholder: 'Pilih Akun',
                        ajax: {
                            url: '/finances-master-data/assets/credit-account/data',
                            dataType: "json",
                            type: "GET",
                            data: params => ({search: params.term}),
                            processResults: data => ({results: data}),
                ...
                async selectedBranch() {
                    ...
                    const response = await $.ajax({
                        type: 'GET',
                        dataType: "JSON",
                        url: `/finances-master-data/assets/branch/selected/${this.editVal.id}`,
                    });
                ...
                async selectedAccount() {
                    ...
                    const response = await $.ajax({
                        type: 'GET',
                        dataType: "JSON",
                        url: `/finances-master-data/assets/account/selected/${this.editVal.id}`,
                    });
                    const option = new Option(response.name, response.id, true, true);
                ...
                async check(id) {
                    showConfirmModal("Anda yakin?", "Aset yang sudah di konfirmasi tidak bisa diubah ataupun dihapus.", "Ya, Konfirmasi!", async () => {
                        try {
                            await axios.post(`/finances-master-data/assets/confirm/${id}`);
